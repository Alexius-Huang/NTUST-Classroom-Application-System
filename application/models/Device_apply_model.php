...
class Device_apply_model extends CI_Model {
  ...
  function update_device_reject_due_to_disabled($id, $device_id) {
    $apply = $this->get_device_apply($id);
    ...
    $update = array(
      'reject_info_zh-TW' => '器材「'.$device['name_zh-TW'].'」已從系統中切換為不開放狀態，請嘗試另訂申請！',
      'reject_info_en-us' => 'Sorry for the inconvenience, however the device："'.$device['name_en-us'].'" is not available now!'
    );
